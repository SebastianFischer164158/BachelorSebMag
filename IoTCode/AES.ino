...
void decryption(int* text, int* key){
    ...
    for (int i = 9; i > 0; i--){
        InvShiftRows(text);
        InvSubBytes(text,16);
        getRoundKey(key,currentRoundKey,i);
        AddRoundKey(text,currentRoundKey);
        InvMixColumns(text);
    }
...
void setup() {
  ...
  // AES SWF file 
    ...
    */
    ...
    unsigned long delta_enc = end_enc - start_enc;
    printf("Encrypted: ");
    printArrayHex(plaintext,16);
    printf("Time to encrypt: %lu \n", delta_enc);
    printf("\n");
    ...
    unsigned long delta_dec = end_dec - start_dec;
